...
export const adminApi = {
  ...
  createUser: (token: string, payload: { email: string; name?: string; role: 'athlete' | 'trainer' | 'admin'; password: string; active?: boolean }) =>
    req<AdminUser>('/v1/admin/users/', { method: 'POST', body: JSON.stringify(payload) }, token),
  patchUser: (token: string, userId: string, payload: { email?: string; name?: string; role?: 'athlete' | 'trainer' | 'admin'; active?: boolean }) =>
