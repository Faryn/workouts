    ...
    <>
      ...
      <div className="card">
        ...
        <div className="row" style={{ flexDirection: 'column', gap: 10 }}>
          {history.map(h => {
            const detail = historyDetails[h.id] ?? null
            ...
            return (
              <div key={h.id} className="card" style={{ marginBottom: 0 }}>
                <div className="row" style={{ justifyContent: 'space-between', alignItems: 'center' }}>
                  ...
                  <button onClick={() => void toggleHistoryDetails(h.id)}>{detail ? 'Hide' : 'Details'}</button>
                </div>
                  ...
                  <div style={{ marginTop: 10 }}>
                    {detail.logged_exercises.map(ex => (
                      <details key={ex.id} style={{ marginBottom: 8 }}>
                        <summary>
                          {(exerciseNameById[ex.exercise_id] ?? ex.exercise_id)}
                        </summary>
